  ...
  methods: {
    ...
    deleteitem(item) {
      this.$dialog
        ...
        .then(() => {
          this.axiosPost('/baselibrary/warehouseDel', {
            id: [item.id],
          }).then(res => {
            if (res.data.code == 1) {
              this.onSearch();
    ...
    goCreat(type, item) {
      this.$router.push({
        ...
        query: { type, item, b_name: 'warehouse_info' },
      });
    },
    ...
    getTableList() {
      let parmars = {
        ...
        b_name: 'warehouse_info',
      };
      if (this.searchValue) {
        Object.assign(parmars, { biz_data: `like|${this.searchValue}` });
      }
      this.axiosPost('/baselibrary/warehouseList', parmars).then(res => {
