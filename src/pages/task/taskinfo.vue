  ...
  <div id="taskcreat">
    ...
    <van-cell class="taskcell" size="large" :border="false">
      <template #title>
        <div @click="selectDDpeo(false, 1)">
          {{ taskform.waitme.name || taskform.waitme_name }}
        </div>
        <div
          style="color: #c8c9cc"
      ...
      <template #icon>
        ...
        <span
          ...
          class="left_icon left_icon_one"
          >{{
            taskform.waitme.name.charAt(taskform.waitme.name.length - 1)
          }}</span
        >
      </template>
      <template #right-icon>
    ...
    >
      <template #title>
        <!-- <div>设置截止时间</div> -->
        <!-- <div>{{ date }}</div> -->
        <div v-if="taskform.start_time && taskform.start_time.length > 0">
    ...
    >
      <template #title>
        <span style="color: #c8c9cc" @click="sontask">子任务</span>
      </template>
      <template #icon>
        <van-icon class="left_icon" name="todo-list-o" size="20" />
      </template>
    </van-cell>
    <div v-if="taskform.child && taskform.child.length > 0" class="childtash">
      <div v-for="item in taskform.child" class="childtash_item">
    ...
    <div class="gongahi">
      计划工时{{ taskform.plan_time || 0 }}小时，实际工时{{
        taskform.actual_time || 0
      }}小时
    </div>
    <!-- 评论 -->
    ...
    <van-cell class="taskcell" size="large" :border="false">
      <template #title>
        <span style="color: #c8c9cc">评论</span>
      </template>
      <template #icon>
        <van-icon class="left_icon" name="comment-o" size="20" />
      </template>
    </van-cell>

    <div class="taskpinglun_con">
      <div
        class="taskfont"
        v-for="(item, index) in taskDiscusslist"
        :key="index"
      >
        <div class="left">
          {{ item.dynamic }}
    ...
    <div class="sureBtn">
      <van-field v-model="progress" center clearable placeholder="请输入评论">
        <template #button>
          <van-button
            style="width: initial"
            size="small"
            type="primary"
            @click="addtaskDiscuss"
            >发 送</van-button
          >
        </template>
      </van-field>
    ...
    />
    <!-- 设置工时 -->
    <van-popup v-model="hourseshow" position="top" :style="{ height: '30%' }">
      <van-form id="searchForm">
        <van-field
...
export default {
  ...
  methods: {
    ...
    addtaskDiscuss() {
      ...
      this.axiosPost('/baselibrary/taskdiscussAdd', {
        enclosure: [],
        gl_id: this.$store.state.taskid,
        operator,
        progress: this.progress,
        type: 3,
      }).then(res => {
        this.progress = '';
        this.gettaskDiscusslist();
      });
    },
    // 获取评论列表
    gettaskDiscusslist() {
      this.axiosPost('/baselibrary/taskDiscusslist', {
        id: this.$store.state.taskid,
        type: 0,
      }).then(res => {
        this.taskDiscusslist = res.data.data;
      });
    },
    ...
    gettaskinfo() {
      ...
      this.axiosPost('/baselibrary/taskDetails', {
        id: this.$store.state.taskid,
      }).then(res => {
        // this.taskform = res.data.data.list[0];
        let taskdata = { ...res.data.data.list[0] };

        this.taskform = JSON.parse(JSON.stringify(taskdata));
        console.log(this.taskform, '54545454');

        this.taskform.man_hour = this.newman_hour;
      });
    },
  },
